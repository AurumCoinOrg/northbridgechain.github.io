...
function readAddressFromTopic(topic?:string){
  if(!topic) return "";
  return "0x" + topic.slice(-40).toLowerCase();
}

function decodeUint8(hex?:string){
  return hexToInt(hex || "0x12");
}
  ...
  useEffect(()=>{
    ...
    async function load(){
      try{
        ...
        for(const [addr, tokenLogs] of byToken.entries()){
          try{
            const [nameHex, symbolHex, decimalsHex, supplyHex] = await Promise.all([
              ...
              rpc("eth_call",[{ to:addr, data:"0x313ce567" },"latest"]).catch(()=>"0x12"),
              rpc("eth_call",[{ to:addr, data:"0x18160ddd" },"latest"]).catch(()=>"0x0")
            ]);
            ...
            for(const log of tokenLogs){
              ...
              const from = readAddressFromTopic(topics[1]);
              const to = readAddressFromTopic(topics[2]);
            ...
            const transferCount = tokenLogs.length;
        ...
        setErr(e?.message || "Failed to scan tokens");
      }finally{
        setLoading(false);
      }
    }
...
const totalVerified = tokens.filter((t) => t.verified).length;
const totalTransfers = tokens.reduce((sum, t) => sum + (Number(t.transfers) || 0), 0);
const mostActiveToken = tokens.length
  ? [...tokens].sort((a, b) => (Number(b.transfers) || 0) - (Number(a.transfers) || 0))[0]
  : null;
  ...
  <option value="name">Sort: Name</option>
      ...
      <div className="statsGrid">
        <div className="statCard">
          <div className="statLabel">Discovered Tokens</div>
          <div className="statValue">{totalTokens.toLocaleString()}</div>
        </div>

        <div className="statCard">
          <div className="statLabel">Verified Tokens</div>
          <div className="statValue">{totalVerified.toLocaleString()}</div>
        </div>

        <div className="statCard">
          <div className="statLabel">Transfer Activity</div>
          <div className="statValue">{totalTransfers.toLocaleString()}</div>
        </div>

        <div className="statCard">
          <div className="statLabel">Most Active Token</div>
          <div className="statValue" style={{fontSize:18}}>
            {mostActiveToken ? mostActiveToken.symbol : "-"}
          </div>
        </div>
      </div>
        ...
        <table style={{width:"100%",minWidth:1080,borderCollapse:"collapse"}}>
          ...
          {loading ? (
            ...
            <tr key={i} style={{borderTop:"1px solid rgba(255,255,255,0.08)"}}>
              ...
              <td style={{padding:"12px 10px"}}>
                <a href={"/token/"+t.address+"/holders"} style={{textDecoration:"none",color:"inherit"}}>
            ...
            <tr>
              <td colSpan={6} style={{padding:"18px 10px",opacity:0.78}}>
                No active tokens found in the scan window.
              </td>
            </tr>
          )}
