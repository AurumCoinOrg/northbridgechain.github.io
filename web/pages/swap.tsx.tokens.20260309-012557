...
export default function SwapPage() {
  ...
  useEffect(() => {
    async function loadQuote() {
      try {
        ...
        if (connected && account) {
          const allow =
            inToken.toLowerCase() === WNBCX.toLowerCase()
              ? ethers.parseUnits(allowWNBCX || "0", 18)
              : ethers.parseUnits(allowNBCX || "0", 18);
    ...
    loadQuote();
  }, [amountIn, inToken, outToken, connected, account, allowWNBCX, allowNBCX, readProvider]);
  ...
  const inLabel = inToken.toLowerCase() === WNBCX.toLowerCase() ? "WNBCX" : "NBCX";
  const outLabel = outToken.toLowerCase() === WNBCX.toLowerCase() ? "WNBCX" : "NBCX";
          ...
          <div style={{ marginTop: 22 }}>
            ...
            <div style={swapBox()}>
              ...
              <select value={inToken} onChange={(e) => setInToken(e.target.value)} style={selectStyle()}>
                <option value={WNBCX}>WNBCX</option>
                <option value={NBCX}>NBCX</option>
              </select>
            </div>
          </div>
